...
int init_DS18B20_Temp1(){
  ...
  if (!sensor1.getAddress(insideThermometer1, 0)){
...
int init_DS18B20_Temp2(){
  ...
  if (!sensor2.getAddress(insideThermometer2, 0)) {
        Serial.println("Unable to find address for Device 0"); 
        return 0;
  }  
  // show the addresses we found on the bus
...
float getTemperature_sensor1(DeviceAddress deviceAddress)
{
  // method 1 - slower
  //Serial.print("Temp C: ");
...
// function to print the temperature for a device
float getTemperature_sensor2(DeviceAddress deviceAddress)
{
  // method 1 - slower
  //Serial.print("Temp C: ");
  //Serial.print(sensors.getTempC(deviceAddress));
