...
unsigned long showLinesFast(uint16_t color) {
...
unsigned long showLines(uint16_t color) {
  unsigned long start, t;
  int           x1, y1, x2, y2,
                w = tft.width(),
                h = tft.height();
...
unsigned long FastLines(uint16_t color1, uint16_t color2) {
  unsigned long start;
